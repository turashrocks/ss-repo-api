...
class SendInvitationToJoinStudio extends Mailable
{
    ...
    {
        if($this->user_exists){
            $url = config('app.client_url').'/settings/studios';
            return $this->markdown('emails.invitations.invite-existing-user')
                ->subject('Invitation to join studio '. $this->invitation->studio->name)
                ->with([
